...
**Status:** v2.12.0 OKR Skills Launch shipping 2026-05-03. Two new skills: `foundation-okr-writer` (`/okr-writer`) and `measure-okr-grader` (`/okr-grader`), both with canonical type and indicator-class enums and three thread-aligned library samples each. Codex adversarial review converged in 3 rounds (HIGH workbench KR3 retroactive scope shrink resolved; canonical enum drift between SKILL.md, TEMPLATE.md, and EXAMPLE.md aligned). Release-prep includes plugin/marketplace bumps, CHANGELOG entry, Release_v2.12.0.md, v2.12.0 manifest, and a stale skill-count reconciliation pass across operational docs (historical What's New entries preserved with inline version refs).
**Last Updated:** 2026-05-03
**Release:** v2.12.0 (commit `a5c000f` grader skill, commit `0173476` release artifacts, count-consistency follow-up commit pending). Previous: v2.11.1 shipped 2026-04-22 (skills.sh CLI compatibility), v2.11.0 shipped 2026-04-18 (Meeting Skills Family + lean-canvas), v2.10.2 prepared 2026-04-14 (never tagged), v2.10.1 shipped 2026-04-13.
...
**v2.13.0 backlog:** Doc consistency overhaul (per user 2026-05-03). Items observed during v2.12.0 release prep but deferred: `utility-pm-skill-builder` SKILL.md catalog table is stale (Foundation listed as 1, Utility listed as 1; actual 8 + 6); `AGENTS/codex/CONTEXT.md` last actively maintained 2026-03-22 at v2.7.0; pre-existing lint-skills-frontmatter failures for utility-pm-skill-validate, utility-slideshow-creator, utility-update-pm-skills (missing references/{TEMPLATE,EXAMPLE}.md); 3 PowerShell CI scripts have parse errors (check-stale-bundle-refs, check-workflow-coverage, check-generated-freshness). Pre-existing v2.12.0 backlog still open: F-29 (lifecycle workflow), F-30 (adoption guide), F-31/32/33/34/35 (sample-automation loop), F-36 (generic family-registration validator).
**Next Step:** (1) Commit count-consistency + CONTEXT-currency follow-up. (2) User runs /ultrareview from their client (cannot be auto-launched). (3) Resolve any /ultrareview findings. (4) Final polish, tag v2.12.0, push origin main + tag, gh release create.
**Process notes for v2.12.0:** Phase 0 Adversarial Review Loop (codified in v2.11.0) ran cleanly. 3 rounds of Codex review before convergence; final round returned 0 findings. Em-dash standing-rule sweep extension included (3 archived audit lines + workbench sample line + README/CHANGELOG cleanup).
